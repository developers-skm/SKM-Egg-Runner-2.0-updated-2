...
import { PowerUpType, HudSnapshot } from '../../types';

interface HUDFastStatsProps {
...
// Everything that changes every animation frame (score/distance/feeds/gems,
// evolution stage, corner-turn/gate/hatching flags, fps, power-up decay)
// lives here, driven by a direct engine subscription instead of App.tsx
// props/state. This keeps the 60fps update loop scoped to this leaf instead
// of re-rendering the whole app tree.
const HUDFastStatsInner: React.FC<HUDFastStatsProps> = ({
  subscribe,
  brownEggsLaid,
  brownEggsCollected,
  isStage2,
  ...
  const [snap, setSnap] = useState<HudSnapshot>(INITIAL_SNAPSHOT);

  useEffect(() => subscribe(setSnap), [subscribe]);

  const {
    score, feeds, gems, distance,
    currentStage, grainsCollected,
    isNearCornerTurn, cornerTurnDirection,
    isHatching,
    activePowerUps
  } = snap;
          ...
          <div className="bg-slate-900/95 border border-slate-800 rounded-2xl backdrop-blur shadow-2xl flex flex-col gap-1.5 w-full relative overflow-hidden" style={{ padding: '10px 12px', minHeight: 72, borderRadius: 16 }}>
            ...
            <div className="flex items-center justify-between text-[8px] font-mono text-white leading-none">
              <span className="flex items-center gap-0.5" title="Feeds Collected">
                🌾 <span className="font-extrabold text-amber-400">Feed:</span> <span className="font-black text-white">{feeds}</span>
              </span>
              <span className="flex items-center gap-0.5" title="Crystal Eggs Collected">
                🥚 <span className="font-extrabold text-[#79ddff]">Eggs:</span> <span className="font-black text-white">{gems}</span>
              </span>
            </div>
...
export const HUDFastStats = React.memo(HUDFastStatsInner);
export default HUDFastStats;
